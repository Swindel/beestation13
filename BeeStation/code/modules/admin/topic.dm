	...
	/////////////////////////////////////new ban stuff
		...
		if(jobban_isbanned(M, "appearance"))
			...
			if("Yes")
				...
				if(M.client)
					jobban_buildcache(M.client)
				ban_unban_log_save("TYPE: Appearance|BANNER: [key_name(usr)]|BANNEE: [key_name(M)]|REASON: [reason];;")
	...
	//JOBBAN'S INNARDS
			...
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
					...
					for(var/job in notbannedlist)
						...
						if(M.client)
							jobban_buildcache(M.client)
						ban_unban_log_save("TYPE: Jobban|BANNER: [key_name(usr)]|BANNEE: [key_name(M)]|JOB: [job]|TIME: [mins]|REASON: [reason];;")
				...
				if("No")
					...
					if(reason)
						...
						for(var/job in notbannedlist)
							...
							if(M.client)
								jobban_buildcache(M.client)
							ban_unban_log_save("TYPE: Perma-Jobban|BANNER: [key_name(usr)]|BANNEE: [key_name(M)]|JOB: [job]|REASON: [reason];;")
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				ban_unban_log_save("TYPE: Ban|BANNER: [key_name(usr)]|BANNEE: [key_name(M)]|TIME: [mins]|REASON: [reason];;")
			...
			if("No")
				...
				if(!DB_ban_record(BANTYPE_PERMA, M, -1, reason))
					to_chat(usr, "<span class='danger'>Failed to apply ban.</span>")
					return
				ban_unban_log_save("TYPE: Permaban|BANNER: [key_name(usr)]|BANNEE: [key_name(M)]|REASON: [reason];;")
